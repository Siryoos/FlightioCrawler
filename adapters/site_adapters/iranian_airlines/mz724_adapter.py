        ...
        if flight_data:
            ...
            if fare_conditions_elem:
                flight_data['fare_conditions'] = {
                    'cancellation': self._extract_text(fare_conditions_elem, '.cancellation'),
            ...
            if seats_elem:
                ...
                flight_data['available_seats'] = self.persian_processor.extract_number(seats_text)
            
            # Mz724 specific: aircraft type
            aircraft_elem = element.select_one('.aircraft-type')
            if aircraft_elem:
                flight_data['aircraft_type'] = self.persian_processor.process_text(aircraft_elem.text.strip())
