"""
Refactored IranAir Aseman adapter using EnhancedPersianAdapter.
"""
...
class IranAirAsemanAdapter(EnhancedPersianAdapter):
    """IranAir Aseman adapter with minimal code duplication."""

    def _get_base_url(self) -> str:
        return "https://www.iranairlines.com"
    ...
    def _parse_flight_element(self, element) -> Optional[Dict[str, Any]]:
        """
        Parse IranAir Aseman specific flight element structure.
        ...
        if flight_data:
            # Add IranAir Aseman specific fields
            config = self.config["extraction_config"]["results_parsing"]

            # IranAir Aseman specific: route information
            route_info = self._extract_text(element, config.get("route_info"))
            if route_info:
                flight_data["route_info"] = self.persian_processor.process_text(
                    route_info
                )

            # IranAir Aseman specific: service class details
            service_details = self._extract_text(element, config.get("service_details"))
            if service_details:
                flight_data["service_details"] = self.persian_processor.process_text(
                    service_details
                )

            # Set airline name
            flight_data["airline"] = "IranAir Aseman"
