...
class AlibabaAdapterRefactored(AirlineCrawler):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        config.update({"base_url": "https://www.alibaba.ir", "site_name": "Alibaba"})
        ...
        try:
            ...
            flight_data = {
                ...
                "flight_number": self._extract_text(
                    element, parsing_config["flight_number"]
                ),
                "departure_time": self._extract_text(
                    element, parsing_config["departure_time"]
                ),
                "arrival_time": self._extract_text(
                    element, parsing_config["arrival_time"]
                ),
                "duration": self._extract_text(element, parsing_config["duration"]),
                "price": self._extract_price(
                    self._extract_text(element, parsing_config["price"])
                ),
                "cabin_class": self._extract_text(
                    element, parsing_config["cabin_class"]
                ),
            }
