...
class FlightParsingStrategy(ABC):
    ...
    @abstractmethod
    def parse_flight_element(self, element: BeautifulSoup, context: ParseContext) -> ParseResult:
    ...
    def _extract_attribute(self, element: BeautifulSoup, selector: str, attribute: str) -> str:
...
class PersianParsingStrategy(FlightParsingStrategy):
    ...
    def parse_flight_element(self, element: BeautifulSoup, context: ParseContext) -> ParseResult:
        ...
        try:
            ...
            flight_data = {
                ...
                "departure_time": self._extract_time_persian(element, config.get("departure_time")),
                "arrival_time": self._extract_time_persian(element, config.get("arrival_time")),
                ...
                "aircraft_type": self._extract_text(element, config.get("aircraft_type")),
    ...
    def _extract_flight_number_persian(self, element: BeautifulSoup, config: Dict[str, Any]) -> str:
    ...
    def _extract_airline_persian(self, element: BeautifulSoup, config: Dict[str, Any]) -> str:
        """Extract airline name with Persian mapping."""
        airline_text = self._extract_text(element, config.get("airline"))
        if airline_text:
            processed_airline = self.persian_processor.normalize_airline_name(airline_text)
    ...
    def _extract_duration_persian(self, element: BeautifulSoup, config: Dict[str, Any]) -> int:
    ...
    def _extract_seat_class_persian(self, element: BeautifulSoup, config: Dict[str, Any]) -> str:
    ...
    def _extract_route_persian(self, element: BeautifulSoup, config: Dict[str, Any]) -> str:
    ...
    def _extract_persian_specific_fields(self, element: BeautifulSoup, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if charter_info:
            fields["is_charter"] = "چارتر" in charter_info or "charter" in charter_info.lower()
            fields["charter_info"] = charter_info
        
        # Baggage information
        baggage_info = self._extract_text(element, config.get("baggage_allowance"))
        if baggage_info:
            fields["baggage_allowance"] = self.persian_processor.process_text(baggage_info)
    ...
    def _process_persian_text_fields(self, flight_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in text_fields:
            if field in flight_data and flight_data[field]:
                flight_data[field] = self.persian_processor.process_text(flight_data[field])
...
class InternationalParsingStrategy(FlightParsingStrategy):
    ...
    def parse_flight_element(self, element: BeautifulSoup, context: ParseContext) -> ParseResult:
        ...
        try:
            ...
            flight_data = {
                ...
                "departure_time": self.extract_time(element, config.get("departure_time")),
                "arrival_time": self.extract_time(element, config.get("arrival_time")),
                "duration_minutes": self._extract_duration_international(element, config),
                "price": self.extract_price(element, config),
            ...
            international_fields = self._extract_international_specific_fields(element, config)
    ...
    def validate_result(self, result: Dict[str, Any]) -> bool:
        ...
        try:
            # Required fields
            required_fields = ["flight_number", "airline", "departure_time", "arrival_time", "price", "currency"]
            ...
            # Price validation
            price = result.get("price", 0)
            if price <= 0 or price > 10000:  # Reasonable range for international flights
                return False
    ...
    def _extract_duration_international(self, element: BeautifulSoup, config: Dict[str, Any]) -> int:
    ...
    def _extract_international_specific_fields(self, element: BeautifulSoup, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if layover_info:
            ...
            fields["has_layover"] = "stop" in layover_info.lower() or "layover" in layover_info.lower()
...
class AggregatorParsingStrategy(FlightParsingStrategy):
    ...
    def parse_flight_element(self, element: BeautifulSoup, context: ParseContext) -> ParseResult:
        ...
        try:
            ...
            aggregator_fields = self._extract_aggregator_specific_fields(element, config)
    ...
    def _extract_currency_aggregator(self, element: BeautifulSoup, config: Dict[str, Any]) -> str:
    ...
    def _extract_duration_aggregator(self, element: BeautifulSoup, config: Dict[str, Any]) -> int:
    ...
    def _extract_aggregator_specific_fields(self, element: BeautifulSoup, config: Dict[str, Any]) -> Dict[str, Any]:
        """Extract aggregator-specific fields."""
        fields = {}
    ...
    def _process_aggregator_text_fields(self, flight_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in text_fields:
            if field in flight_data and flight_data[field]:
                ...
                if any(persian_char in text for persian_char in 'ابپتثجچحخدذرزژسشصضطظعغفقکگلمنوهی'):
...
class ParsingStrategyFactory:
    ...
    @staticmethod
    def create_strategy(strategy_type: str, config: Dict[str, Any]) -> FlightParsingStrategy:
