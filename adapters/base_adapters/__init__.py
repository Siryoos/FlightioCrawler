...
class AdapterUtils:
    ...
    @staticmethod
    def merge_flight_data(base_data: Dict[str, Any], additional_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def validate_config(config: Dict[str, Any]) -> List[str]:
        ...
        if "extraction_config" in config:
            ...
            if "search_form" in extraction_config:
                ...
                required_form_fields = ["origin_field", "destination_field", "date_field"]
            ...
            if "results_parsing" in extraction_config:
                ...
                if "container" not in results_config:
                    errors.append("Missing container selector in results_parsing config")
...
class ErrorReportingHelper:
    ...
    def log_error_report(report: Dict[str, Any], logger: logging.Logger) -> None:
        ...
        """
        error_msg = f"[{report['adapter']}] {report['error_type']}: {report['error_message']}"
        ...
        if report.get("stack_trace"):
            logger.debug(f"Stack trace for {report['adapter']}: {report['stack_trace']}")
