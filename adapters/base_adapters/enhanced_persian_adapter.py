...
class EnhancedPersianAdapter(EnhancedBaseCrawler):
            ...
            if language_config.get("selector"):
                ...
                if language_config.get("persian_option"):
                    await self.page.click(
                        language_config["persian_option"], timeout=5000
                    )
    ...
    async def _fill_search_form(self, search_params: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if "date_field" in form_config:
                await self._fill_date_field_persian(
                    search_params["departure_date"], form_config
                )
            ...
            if "seat_class" in search_params and "class_field" in form_config:
                await self._fill_seat_class_persian(
                    search_params["seat_class"], form_config
                )

            # Trip type with Persian processing
            if "trip_type" in search_params and "trip_type_field" in form_config:
                await self._fill_trip_type_persian(
                    search_params["trip_type"], form_config
                )
                ...
                try:
                    ...
                    if suggestions:
                        ...
                        for suggestion in suggestions:
                            ...
                            if (
                                airport_code.upper() in text.upper()
                                or processed_airport in text
                            ):
                    ...
                    self.logger.debug(
                        f"Persian autocomplete handling failed for {field_type}: {e}"
                    )
    ...
    async def _fill_date_field_persian(
        self, date_str: str, form_config: Dict[str, Any]
    ) -> None:
    ...
    async def _fill_passenger_fields_persian(
        ...
        try:
            ...
            if "passengers" in search_params and "passengers_field" in form_config:
                ...
                await self.page.select_option(
                    form_config["passengers_field"], persian_count
                )
    ...
    async def _fill_seat_class_persian(
        self, seat_class: str, form_config: Dict[str, Any]
    ) -> None:
    ...
    async def _fill_trip_type_persian(
        ...
        try:
            ...
            if trip_type_field.startswith("select"):
                ...
                await self.page.click(
                    f"{trip_type_field}[value='{processed_trip_type}']"
                )
    ...
    async def _fill_additional_fields_persian(
        self, search_params: Dict[str, Any], form_config: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            if (
                search_params.get("charter_preferred")
                and "charter_field" in form_config
            ):
            ...
            if (
                ...
                await self.page.select_option(
                    form_config["preferred_airline_field"], persian_airline
                )
    ...
    def _manual_duration_parsing_persian(self, duration_text: str) -> int:
        ...
        try:
            ...
            if "دقیقه" in duration_text:
                minutes_part = (
                    converted.split("ساعت")[-1] if "ساعت" in converted else converted
                )
        ...
        for field in ["origin", "destination"]:
            if field in search_params:
                ...
                if not self._is_valid_iranian_airport(airport):
                    self.logger.warning(
                        f"Airport {airport} may not be valid for Persian flights"
                    )
    ...
    def _is_valid_iranian_airport(self, airport: str) -> bool:
        ...
        return airport.upper() in [a.upper() for a in iranian_airports] or any(
            a in airport for a in iranian_airports
        )
