...
class AdapterRegistry:
    ...
    def search_adapters(self, query: str) -> List[str]:
        ...
        for name in self._adapters.keys():
            ...
            # Search in metadata
            metadata = self.get_metadata(name)
            if (query_lower in metadata.get("description", "").lower() or
                query_lower in metadata.get("airline_name", "").lower() or
                query_lower in str(metadata.get("features", [])).lower()):
                results.append(name)
...
class AdapterFactory:
    ...
    def _register_persian_adapter(self, adapter_info: Dict[str, Any]) -> None:
        ...
        # Create metadata
        metadata = {
            "type": "persian" if not adapter_info.get("is_aggregator") else "aggregator",
            "airline_name": adapter_info["airline_name"],
            "description": adapter_info["description"],
            "features": adapter_info["features"],
            "currency": adapter_info["currency"],
    ...
    def _discover_adapters(self) -> None:
        ...
        try:
            ...
            self._discover_adapters_in_path("adapters.site_adapters.iranian_aggregators")
            self._discover_adapters_in_path("adapters.site_adapters.international_aggregators")
