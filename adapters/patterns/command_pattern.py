...
class Command(ABC):
    ...
    def __init__(self, name: str, description: str = "", priority: CommandPriority = CommandPriority.NORMAL):
        self.name = name
    ...
    def can_retry(self) -> bool:
        ...
        return (self.context.retry_count < self.context.max_retries and 
                self.context.status in [CommandStatus.FAILED, CommandStatus.CANCELLED])
...
class CrawlSiteCommand(Command):
    ...
    def __init__(self, site_url: str, adapter_name: str, search_params: Dict[str, Any], 
                 priority: CommandPriority = CommandPriority.NORMAL):
        super().__init__(
...
class ValidateDataCommand(Command):
    ...
    def __init__(self, data: List[Dict], validation_rules: Dict[str, Any],
    ...
    def _validate_item(self, item: Dict) -> Tuple[bool, List[str]]:
        ...
        for field, expected_type in field_types.items():
            if field in item and item[field] is not None:
                if expected_type == 'number' and not isinstance(item[field], (int, float)):
...
class MacroCommand(Command):
    ...
    def __init__(self, name: str, commands: List[Command], 
                 ...
                 priority: CommandPriority = CommandPriority.NORMAL):
        super().__init__(name, f"Macro command with {len(commands)} operations", priority)
            ...
            for cmd in self.commands:
                ...
                if result.success:
                    ...
                    if self.stop_on_failure:
                        logger.error(f"Macro command stopped due to failure in: {cmd.name}")
                        break
                    else:
                        logger.warning(f"Macro command continuing despite failure in: {cmd.name}")
...
class CommandInvoker:
    ...
    def queue_command(self, command: Command) -> None:
        ...
        logger.info(f"Queued command: {command} (priority: {command.context.priority.name})")
        ...
        try:
            ...
            if result.success:
                ...
                if command.can_retry():
                    ...
                    logger.info(f"Retrying command: {command} (attempt {command.context.retry_count})")
    ...
    def undo_last_command(self) -> Optional[CommandResult]:
        ...
        for command in reversed(self.command_history):
            if (command.context.status == CommandStatus.COMPLETED and 
                command.can_undo()):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        completed = sum(1 for cmd in self.command_history 
                       if cmd.context.status == CommandStatus.COMPLETED)
        failed = sum(1 for cmd in self.command_history 
                    if cmd.context.status == CommandStatus.FAILED)
        
        # Calculate average execution time
        execution_times = [cmd.context.elapsed_time() for cmd in self.command_history 
...
def create_crawl_and_save_workflow(site_configs: List[Dict[str, Any]], 
                                 search_params: Dict[str, Any],
                                 validation_rules: Dict[str, Any],
                                 table_name: str = "flight_data") -> MacroCommand:
...
def create_validation_workflow(data: List[Dict], validation_rules: Dict[str, Any]) -> MacroCommand:
...
def get_command_invoker(max_workers: int = 4, enable_history: bool = True) -> CommandInvoker:
