...
class DatabaseManager(ResourceManager, metaclass=SingletonMeta):
    ...
    def initialize(self, connection_info: DatabaseConnectionInfo) -> None:
        ...
        logger.info(f"DatabaseManager initialized with database: {connection_info.database_path}")
        ...
        with self._lock:
            if current_thread not in self._connections:
                try:
                    ...
                    logger.debug(f"Created new database connection for thread {current_thread.name}")
        ...
        with self._lock:
            for thread, conn in self._connections.items():
                try:
                    ...
                    logger.error(f"Error closing connection for thread {thread.name}: {e}")
...
class ConfigurationManager(ResourceManager, metaclass=SingletonMeta):
    ...
    def initialize(self, config_files: Union[str, Path, List[Union[str, Path]]]) -> None:
        ...
        logger.info(f"ConfigurationManager initialized with {len(self._config_files)} config files")
...
class CacheManager(ResourceManager, metaclass=SingletonMeta):
    ...
    def _schedule_cleanup(self) -> None:
        ...
        self._cleanup_timer = threading.Timer(300, self._cleanup_expired)  # Every 5 minutes
...
class LoggingManager(ResourceManager):
    ...
    def initialize(self, log_level: str = "INFO", log_file: Optional[str] = None) -> None:
