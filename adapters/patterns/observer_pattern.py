...
class EventObserver(ABC):
    ...
    def is_interested_in(self, event_type: EventType) -> bool:
        ...
        return self.is_active and (not self.observed_events or event_type in self.observed_events)


class LoggingObserver(EventObserver):
    """Observer that logs events to a logger."""
    
    def __init__(self, name: str = "LoggingObserver", logger_name: Optional[str] = None):
        super().__init__(name)
    ...
    async def on_event(self, event: CrawlerEvent) -> None:
        ...
        with self._lock:
            ...
            if event.event_type == EventType.CRAWL_STARTED:
                ...
                if duration:
                    current_avg = self.metrics["performance_stats"]["avg_crawl_duration"]
                    total_crawls = self.metrics["crawl_stats"]["successful_crawls"]
                    new_avg = ((current_avg * (total_crawls - 1)) + duration) / total_crawls
                    ...
                    if self.metrics["performance_stats"]["min_crawl_duration"] != float('inf'):
                        self.metrics["performance_stats"]["min_crawl_duration"] = min(
                            self.metrics["performance_stats"]["min_crawl_duration"], 
                            duration
                        )
                    else:
                        self.metrics["performance_stats"]["min_crawl_duration"] = duration
...
class DatabaseObserver(EventObserver):
    ...
    def __init__(self, name: str = "DatabaseObserver", connection_string: Optional[str] = None):
        super().__init__(name)
...
class AlertObserver(EventObserver):
    ...
    def __init__(self, name: str = "AlertObserver", alert_callback: Optional[Callable] = None):
        super().__init__(name)
...
class EventSubject:
    ...
    def __init__(self, name: str = "CrawlerEventSubject"):
        ...
        self.executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="event_notify")
    ...
    async def notify(self, event: CrawlerEvent) -> None:
        ...
        with self._lock:
            ...
            current_observers = [obs for obs in self.observers if obs.is_interested_in(event.event_type)]
        ...
        if current_observers:
            ...
            for observer in current_observers:
                try:
                    ...
                    logger.error(f"Error creating task for observer {observer.name}: {e}")
    ...
    def get_event_history(self, event_type: Optional[EventType] = None, limit: Optional[int] = None) -> List[CrawlerEvent]:
...
class CrawlerEventSystem:
    ...
    async def emit_event(self, event_type: EventType, source: str, data: Optional[Dict[str, Any]] = None, 
                        correlation_id: Optional[str] = None, severity: str = "info") -> None:
...
async def emit_crawl_started(source: str, data: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None) -> None:
    """Emit crawl started event."""
    await get_event_system().emit_event(EventType.CRAWL_STARTED, source, data, correlation_id)


async def emit_crawl_completed(source: str, data: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None) -> None:
    """Emit crawl completed event."""
    await get_event_system().emit_event(EventType.CRAWL_COMPLETED, source, data, correlation_id)


async def emit_crawl_failed(source: str, data: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None) -> None:
    """Emit crawl failed event."""
    await get_event_system().emit_event(EventType.CRAWL_FAILED, source, data, correlation_id, "error")


async def emit_flight_extracted(source: str, data: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None) -> None:
    """Emit flight extracted event."""
    await get_event_system().emit_event(EventType.FLIGHT_EXTRACTED, source, data, correlation_id)


async def emit_error_occurred(source: str, data: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None) -> None:
    """Emit error occurred event."""
    await get_event_system().emit_event(EventType.ERROR_OCCURRED, source, data, correlation_id, "error") 