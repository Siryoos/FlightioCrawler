...
class EnhancedMonitoring:
    ...
    def record_request_success(
        ...
        if site in self.crawler_metrics:
            ...
            metrics.avg_response_time = (
                metrics.total_duration / metrics.successful_requests
            )
            ...
            self.crawler_avg_response_time.labels(site=site).set(
                metrics.avg_response_time
            )
    ...
    def record_request_error(
        self, site: str, error_type: str, duration: float = 0.0, error_message: str = ""
    ):
    ...
    def record_price_change(
        self, site: str, old_price: float, new_price: float, route: str = "unknown"
    ):
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "system": {
                "total_requests": sum(
                    m.total_requests for m in self.crawler_metrics.values()
                ),
                "total_errors": sum(
                    m.error_count for m in self.crawler_metrics.values()
                ),
                "active_crawlers": len(
                    [
                        m
                        ...
                        and datetime.now() - m.last_request_time < timedelta(minutes=5)
                    ]
                ),
            },
            "alerts": [],
        ...
        for site, metrics in self.crawler_metrics.items():
            ...
            if metrics.circuit_breaker_open:
                summary["alerts"].append(
                    {"type": "circuit_breaker_open", "site": site, "value": True}
                )
    ...
    def get_performance_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            for record in recent_data:
                ...
                # System trends
                trends["system_trends"]["total_requests"].append(
                    record["system"]["total_requests"]
                )
                trends["system_trends"]["total_errors"].append(
                    record["system"]["total_errors"]
                )
                trends["system_trends"]["active_crawlers"].append(
                    record["system"]["active_crawlers"]
                )
            ...
            for site, data in crawler_data.items():
                if data:
                    ...
                    avg_response_time = sum(d["avg_response_time"] for d in data) / len(
                        data
                    )
