...
class EnhancedErrorHandler:
    ...
    async def _check_circuit_breaker(self, domain: str) -> None:
        ...
        try:
            ...
            # Get circuit config
            circuit_config = self.retry_strategies.get(domain, self.default_circuit_config)
            ...
            if current_state == CircuitState.CLOSED:
                ...
                if datetime.now() - open_time >= timedelta(seconds=circuit_config.recovery_timeout):
    ...
    async def _handle_retry(
        ...
        try:
            ...
            self.logger.info(f"Retrying {domain} in {delay} seconds (attempt {error_record.retry_count + 1})")
            ...
            try:
                ...
                if error_record.retry_count >= error_record.max_retries:
                    ...
                    self.logger.error(f"Max retries exceeded for {domain}, moved to DLQ")
                else:
                    # Schedule next retry
                    await self._handle_retry(domain, error_record, retry_strategy)
    ...
    async def _execute_error_callbacks(self, domain: str, error_record: ErrorRecord) -> None:
