...
from site_crawlers import SnapptripCrawler
...
def sample_html():
    ...
    <div class='flight-item'>
        <span class='airline-name'>SnapAir</span>
        <span class='flight-number'>SN100</span>
        <span class='departure-time'>08:00</span>
        <span class='arrival-time'>10:00</span>
        <span class='price'>5,000,000 ریال</span>
...
def crawler(monkeypatch, sample_html):
    ...
    crawler = SnapptripCrawler(DummyRateLimiter(), None, DummyMonitor(), DummyErrorHandler())
...
async def test_parse_search_results(crawler):
    ...
    assert results[0]['airline'] == 'SnapAir'
    assert results[0]['flight_number'] == 'SN100'
