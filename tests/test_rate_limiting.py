...
class TestRateLimitMiddleware:
    ...
    async def test_endpoint_type_detection(self, rate_limit_middleware):
        ...
        assert (
            rate_limit_middleware._get_endpoint_type("/search/intelligent") == "search"
        )
    ...
    async def test_redis_rate_limit_check(self, rate_limit_middleware):
        ...
        # Test within limits
        rate_limit_middleware.redis.pipeline().execute = AsyncMock(
            return_value=[5, 50, 2]
        )
    ...
    async def test_local_rate_limit_fallback(self, rate_limit_middleware):
        ...
        # Test first request
        result = await rate_limit_middleware._check_local_rate_limits(
            client_ip, endpoint_type, config
        )
        assert result["allowed"] is True
        ...
        for i in range(config["requests_per_minute"]):
            await rate_limit_middleware._check_local_rate_limits(
                client_ip, endpoint_type, config
            )
...
class TestRateLimitManager:
    ...
    async def test_get_rate_limit_stats(self, rate_limit_manager):
        ...
        # Mock Redis response
        rate_limit_manager.redis.hgetall = AsyncMock(
            return_value={
    ...
    async def test_get_client_status(self, rate_limit_manager):
        ...
        rate_limit_manager.redis.pipeline().execute = AsyncMock(
            return_value=[5, 50, 2, 55, 3550, 58]
        )

        status = await rate_limit_manager.get_client_rate_limit_status(
            "192.168.1.100", "search"
        )
    ...
    async def test_get_blocked_clients(self, rate_limit_manager):
        ...
        rate_limit_manager.redis.keys = AsyncMock(
            return_value=[
