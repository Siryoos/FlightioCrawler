...
class IntelligentSearchEngine:
    ...
    async def optimize_search_strategy(self, search_params: Dict, optimization: SearchOptimization) -> Dict:
        ...
        if optimization.enable_multi_route:
            results["connecting_flights"] = await self.discover_connecting_flights(
                search_params["origin"], search_params["destination"], search_params["departure_date"]
            )
        if optimization.enable_date_range:
            results["date_range"] = await self.search_date_range(search_params, optimization.date_range_days)
        ...
        if optimization.enable_seasonal_tracking:
            results["seasonal_trends"] = await self.get_seasonal_price_trends(search_params["origin"] + "-" + search_params["destination"])
        return results

    async def discover_connecting_flights(self, origin: str, destination: str, date: str) -> List[ConnectingFlight]:
        ...
        origin_flights = await self.main_crawler.crawl_all_sites({"origin": origin, "destination": destination, "departure_date": date})
        connecting_flights = []
        for origin_flight in origin_flights:
            ...
            connecting_flight_list = await self.main_crawler.crawl_all_sites({"origin": destination, "destination": final_destination, "departure_date": date})
            for connecting_flight in connecting_flight_list:
                # Calculate layover duration
                layover_duration = (datetime.fromisoformat(connecting_flight["departure_time"]) - datetime.fromisoformat(origin_flight["arrival_time"])).total_seconds() / 60
                if 60 <= layover_duration <= 240:  # Filter by layover duration (1-4 hours)
                    connecting_flights.append(ConnectingFlight(
                        origin_flight=origin_flight,
                        connecting_flight=connecting_flight,
                        layover_duration_minutes=int(layover_duration),
                        total_price=origin_flight["price"] + connecting_flight["price"],
                        total_duration_minutes=int(layover_duration + (datetime.fromisoformat(connecting_flight["arrival_time"]) - datetime.fromisoformat(origin_flight["departure_time"])).total_seconds() / 60),
    ...
    async def search_date_range(self, search_params: Dict, days_range: int = 3) -> Dict:
        ...
        for i in range(days_range):
            ...
            date_results[current_date] = await self.main_crawler.crawl_all_sites({**search_params, "departure_date": current_date})
        return date_results

    async def detect_class_upgrades(self, flights: List[Dict], threshold: float = 20.0) -> List[Dict]:
        ...
        for flight in flights:
            if flight.get("seat_class") == "economy":
                ...
                if (business_price - flight["price"]) / flight["price"] * 100 <= threshold:
                    upgrades.append({**flight, "upgrade_price": business_price})
        return upgrades

    async def get_seasonal_price_trends(self, route: str, historical_months: int = 12) -> Dict:
        """Get seasonal price trends for a route."""
        # Query historical data from db_manager
        historical_data = await self.db_manager.get_historical_prices(route, historical_months)
    ...
    async def recommend_alternative_dates(self, search_params: Dict) -> List[Dict]:
        ...
        historical_data = await self.db_manager.get_historical_prices(search_params["origin"] + "-" + search_params["destination"])
        return [{"date": date, "price": price} for date, price in historical_data.items() if price < search_params.get("price", float("inf"))] 