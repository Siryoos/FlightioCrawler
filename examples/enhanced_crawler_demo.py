    ...
    if all_results:
        ...
        for airline, flights in by_airline.items():
            if flights:
                ...
                print(f"  - {airline}: {len(flights)} پرواز، میانگین قیمت: {avg_price:,.0f} ریال")
...
async def demo_international_adapters():
    ...
    print(f"جستجوی پرواز بین‌المللی: {search_params['origin']} → {search_params['destination']}")
    print(f"تعداد مسافران: {search_params['passengers']}, کلاس: {search_params['seat_class']}")
...
def demo_factory_features():
    ...
    print(f"  متادیتا: {json.dumps(mahan_info['metadata'], ensure_ascii=False, indent=2)}")
