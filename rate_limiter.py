...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        rate_config = RATE_LIMIT_CONFIGS.get(endpoint_type, RATE_LIMIT_CONFIGS["default"])
        ...
        effective_config = {
            "requests_per_minute": int(rate_config["requests_per_minute"] * user_multiplier),
            "requests_per_hour": int(rate_config["requests_per_hour"] * user_multiplier),
            "burst_limit": int(rate_config["burst_limit"] * user_multiplier)
        }
        
        # Check rate limits
        rate_limit_result = await self._check_rate_limits(client_ip, endpoint_type, effective_config)
    ...
    async def _check_rate_limits(self, client_ip: str, endpoint_type: str, config: Dict) -> Dict:
        ...
        if self.redis:
            return await self._check_redis_rate_limits(minute_key, hour_key, burst_key, config)
        else:
            return await self._check_local_rate_limits(client_ip, endpoint_type, config)
    
    async def _check_redis_rate_limits(self, minute_key: str, hour_key: str, burst_key: str, config: Dict) -> Dict:
    ...
    async def _check_local_rate_limits(self, client_ip: str, endpoint_type: str, config: Dict) -> Dict:
    ...
    async def _create_rate_limit_response(self, rate_limit_result: Dict) -> JSONResponse:
        ...
        headers = {
            ...
            "Retry-After": str(max(1, rate_limit_result["reset_time"] - int(time.time())))
        }
    ...
    def _add_rate_limit_headers(self, response: Response, rate_limit_result: Dict):
        ...
        if rate_limit_result.get("allowed"):
            ...
            if "burst_remaining" in rate_limit_result:
                response.headers["X-RateLimit-Remaining-Burst"] = str(rate_limit_result["burst_remaining"])
...
class RateLimitManager:
    ...
    async def get_client_rate_limit_status(self, client_ip: str, endpoint_type: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "minute_remaining": config["requests_per_minute"] - int(results[0] or 0),
    ...
    async def reset_client_rate_limits(self, client_ip: str, endpoint_type: Optional[str] = None) -> Dict:
    ...
    async def update_rate_limit_config(self, endpoint_type: str, config: Dict) -> Dict:
        ...
        logger.info(f"Updated rate limit config for {endpoint_type}: {old_config} -> {RATE_LIMIT_CONFIGS[endpoint_type]}")
        ...
        try:
            ...
            for key in all_keys:
                ...
                if len(parts) >= 4:
                    ...
                    if limit_type == "minute" and count >= config["requests_per_minute"]:
